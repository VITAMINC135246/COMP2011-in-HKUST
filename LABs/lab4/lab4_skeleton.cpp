...
void calculateMaxMinAverage(const int scores[], int count, int &max, int &min,
  ...
  for(int j = 0; j < count; j++){
    ...
    {
      max = scores[j];
      min = scores[j];//initialize max and min
    }
    ...
    average += scores[j];//score add up for each iteration
  ...
  for(int i = 0; i < count; i++){
    for(int j = 0; j < count - i - 1; j++){
      if (scores[j] < scores[j + 1])
      {
        ...
        scores[j] = scores[j + 1];
        scores[j + 1] = temp;
      }//swap if satiesfied the decresing order condition
    }
  }
}
...
void assignLetterGrades(const int CUTOFFS[], const int scores[],
  ...
  for (int i = 0; i < count; i++)
  {
    (scores[i] >= CUTOFFS[0])? letterGrades[i] = 'A'://"? continuous judgment"
    (scores[i] >= CUTOFFS[1])? letterGrades[i] = 'B': 
    (scores[i] >= CUTOFFS[2])? letterGrades[i] = 'C':
    (scores[i] >= CUTOFFS[3])? letterGrades[i] = 'D': letterGrades[i] = 'F';
  }
