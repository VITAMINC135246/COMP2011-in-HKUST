...
{
  // If title is a null pointer, treat it as an empty string.
  if (title == nullptr)
  {
    title = "";
  }
  ...
  // allocate memory on the heap for dynamic usage
  char *new_title = new char[len];

  // copy
  strcpy(new_title, title);

  // free up the dynamic memory pointed to by the old `note.title`.
  delete[] note.title;
...
{
  // If content is a null pointer, treat it as an empty string.
  if (content == nullptr)
  {
    content = "";
  }
  ...
  // allocate memory on the heap for dynamic usage
  char *new_content = new char[len];

  // copy
  strcpy(new_content, content);

  // free up the dynamic memory pointed to by the old `note.content`.
  delete[] note.content;
...
bool remove_note(Notes &notes, const char title[])
{
  ...
  // find the `Note` object to be removed by using strstr.
  int object = -1;
  unsigned title_len = strlen(title);

  for (unsigned int i = 0; i < notes.note_array_count; ++i)
  {
    const char *pos = strstr(notes.note_array[i].title, title);
    if (pos && pos == notes.note_array[i].title &&
        strlen(notes.note_array[i].title) == title_len) // if: findable + exactly at the beginning of the string(start at same time) + eqaul length(end at same time)
    {
  ...
  // use `set_note_title` and `set_note_content` to move the `Note` objects after it to the front by 1 slot, covering the `Note` object to be removed.
  for (unsigned int i = object; i + 1 < notes.note_array_count; ++i)
  {
    set_note_title(notes.note_array[i], notes.note_array[i + 1].title);
    set_note_content(notes.note_array[i], notes.note_array[i + 1].content);
  }

  // clean up the last *initialized* `Note` object and update the `note_array_count`.
...
{
  // check if the new title clashes with the title of an existing note.
  for (int i = 0; i < notes.note_array_count; ++i)
  {
  ...
  // if no clashing, the rename by using set_note_title
  for (unsigned int i = 0; i < notes.note_array_count; ++i)
  {
    if (strcmp(title, notes.note_array[i].title) == 0)
    {
      set_note_title(notes.note_array[i], new_title);
      return true;
    }
  }
...
bool edit_note(Notes &notes, const char title[], const char new_content[])
{
  ...
  // directly iterate and find and change
  for (unsigned int i = 0; i < notes.note_array_count; ++i)
  {
    if (strcmp(title, notes.note_array[i].title) == 0)
    {
...
bool append_note_content(Notes &notes, const char title[], const char content_to_append[])
{
  ...
  for (unsigned int i = 0; i < notes.note_array_count; ++i)
  {
    if (strcmp(title, notes.note_array[i].title) == 0)
    {
      ...
      //length calculating of char
      int original_len = strlen(notes.note_array[i].content) ;
      int add_len = strlen(content_to_append) ;
      int new_len = original_len + add_len + 1;

      //combine
      char * new_content = new char[new_len];
      strcpy(new_content, notes.note_array[i].content);
