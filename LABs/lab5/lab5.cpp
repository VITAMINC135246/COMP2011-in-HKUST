...
bool validCharacters(const char address[], int index) {
    ...
    if (address[index] != '.' && (address[index] < '0' || address[index] > '9')) {
        return false;
    }
...
bool validDotUsage(const char address[], int index, int dotCount){
    ...
    if (address[index] == '.'){
        ...
        if (address[index+1] == '\0'){
            return false;
        }// case of trailing dot

        if ((index>0 && address[index-1]=='.') || address[index+1]=='.'){
            return false;
        }// case of consecutive dot
}
